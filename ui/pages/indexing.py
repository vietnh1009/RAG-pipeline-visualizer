...
def _run_pipeline_and_show_progress() -> None:
    ...
    if st.session_state.get("_pipeline_running") and not process_btn:
        ...
        # Bỏ qua Chunking → Embedding và Vector DB cũng vô nghĩa, tắt luôn.
        _skip_chunk = bool(st.session_state.get("_last_chunk_skip", False))
        _skip_embed = _skip_chunk or emb_cfg.get("skip")
        _skip_vdb   = _skip_chunk or vdb_cfg.get("skip")
        ...
        # Xoá kết quả chunk cũ để tab Chunking không hiện dữ liệu của lần trước
        if _skip_chunk:
            for _k in ("chunks", "embed_result", "vdb_result"):
        ...
        # ── Bước 2: Chunking ──────────────────────────────────────────────────
        if _skip_chunk:
            st.info("⏭️ Đã bỏ qua bước Chunking — pipeline dừng sau Loader.", icon="ℹ️")
            st.session_state["active_tab"] = 0
        elif docs and not stop_event.is_set():
...
# ═════════════════════════════════════════════════════════════════════════════
# Giữ widget state qua các lượt chạy KHÔNG render sidebar
#
# Streamlit dọn `st.session_state[key]` của widget nào KHÔNG được render trong
# một lượt chạy. Trang này có "fast path": khi `_pipeline_running` bật, nó gọi
# _run_pipeline_and_show_progress() rồi return NGAY — toàn bộ widget sidebar
# không được vẽ → Streamlit dọn sạch lựa chọn của người dùng.
#
# Hậu quả: sau mỗi lần bấm Process, mọi lựa chọn bị reset về mặc định —
# checkbox "Bỏ qua bước Embedding" bật lại thành tắt, loader/chunking/embedding
# provider quay về giá trị đầu. Pipeline vẫn chạy đúng vì nó đọc `_last_*_cfg`
# (key thường, không phải widget key, nên không bị dọn) — chỉ sidebar sai.
#
# Cách xử lý: gán lại chính giá trị đó vào session_state — thao tác này đánh dấu
# key là "do người dùng đặt", miễn nhiễm với cơ chế dọn widget.
#
# QUAN TRỌNG — chỉ gọi ở lượt chạy KHÔNG vẽ widget nào (nhánh fast path).
# Streamlit cảnh báo khi một key vừa được đặt qua Session State API TRONG CÙNG
# lượt chạy mà widget đó được tạo kèm `value=`. Trong repo này có 24 widget khai
# báo cả `key=` lẫn `value=`, nên gọi hàm này ở mọi lượt sẽ làm log ngập cảnh báo.
# Ở lượt fast path không widget nào được tạo → không có cảnh báo, mà state vẫn được giữ.
# DANH SÁCH TƯỜNG MINH, không lọc theo tiền tố.
#
# Bản trước lọc theo tiền tố ("vdb_", "chunk_"...) và loại trừ theo tiền tố
# ("btn_", "test_"). Cách đó làm hỏng 3 nút bấm có chữ btn/test ở CUỐI key:
#     vdb_test_btn (ui/results/vdb_results.py)
#     chunk_prev, chunk_next (ui/results/chunking_results.py)
# Streamlit CẤM gán session_state cho key của st.button →
#     StreamlitValueAssignmentNotAllowedError
# Lỗi nổ lúc TẠO widget chứ không phải lúc gán, nên try/except vô dụng —
# chỉ có cách không đụng vào chúng ngay từ đầu.
#
# Danh sách dưới đây gồm các widget CÓ TRẠNG THÁI trong ui/settings/.
# Thêm widget sidebar mới thì bổ sung key vào đây, nếu không lựa chọn của
# nó sẽ bị reset sau mỗi lần Process. Sinh lại danh sách bằng cách quét AST
# ui/settings/*.py, lấy key của checkbox/selectbox/number_input/text_input/
# radio/slider/select_slider — và TUYỆT ĐỐI không lấy key của st.button.
_PERSIST_KEYS = frozenset({
    # chunking
    "chunk_skip", "sel_chunking_strategy", "sel_format_type",
    "sel_split_large_sections", "fa_chunk_size", "fa_chunk_overlap",
    "ollama_embed_url",
    # embedding
    "emb_skip", "emb_embed_all", "emb_enable_sparse", "emb_max_preview",
    "emb_sparse_method", "sel_emb_provider", "sel_emb_model", "sel_emb_device",
    "sel_emb_dtype", "sel_emb_batch", "ollama_emb_url",
    # loader
    "sel_pdf_strategy", "sel_odl_mode", "odl_force_ocr", "odl_hybrid_port",
    "odl_struct_tree",
    # vector db
    "vdb_skip", "sel_vdb_provider", "vdb_collection_name", "vdb_distance",
    "vdb_force_reindex", "vdb_qdrant_url", "vdb_qdrant_on_disk",
    "vdb_weaviate_url", "vdb_pg_conn", "vdb_pg_dist", "vdb_pc_cloud",
    "vdb_pc_region",
})

# Key sinh động theo f-string trong ui/settings/loader.py (panel mô tả ảnh VLM).
# CHỈ liệt kê widget có trạng thái — `test_ollama_{prefix}` là st.button, bỏ qua.
_PERSIST_DYNAMIC = (
    "_vlm_describe_", "_vlm_model_", "_vlm_provider_", "_vlm_url_",
)


def _persist_sidebar_widget_state() -> None:
    """Giữ lựa chọn sidebar qua lượt chạy không render widget.

    CHỈ gọi từ nhánh fast path (_pipeline_running). Xem chú thích ở trên.
    """
    for key in list(st.session_state.keys()):
        if not isinstance(key, str):
            continue
        if key in _PERSIST_KEYS or key.startswith(_PERSIST_DYNAMIC):
...
def _page_indexing():
    ...
    if st.session_state.get("_pipeline_running"):
        # CHỈ giữ state ở đúng lượt này — lượt mà sidebar KHÔNG được vẽ.
        # Gọi ở mọi lượt sẽ sinh cảnh báo:
        #   "The widget with key ... was created with a default value but also
        #    had its value set via the Session State API."
        # vì 24 widget trong ui/settings/ khai báo cả `key=` lẫn `value=`.
        _persist_sidebar_widget_state()
    ...
    with st.sidebar:
        ...
        # chunk_skip do checkbox trong render_chunking_settings() ghi.
        # Đọc được ở đây dù chunking render SAU, vì widget key tồn tại trong
        # session_state ngay khi người dùng tick (trước lượt rerun kế tiếp).
...
| `opendataloader` | ⚡⚡ Nhanh (fast) | ✅✅ Rất tốt | ✅ | **Accuracy #1 (0.90) ở hybrid mode** · không cần GPU · ⚠️ cần Java 11+ |
""")
...
---
**🟣 `opendataloader` — Chính xác nhất theo benchmark, không cần GPU**

Parser mã nguồn mở chạy trên nền Java, phân tích layout bằng thuật toán XY-Cut++ (fast mode)
hoặc định tuyến sang backend AI (hybrid mode). Trả về cả bounding box của từng khối nội dung.

- ✅ **fast mode** — 0.05s/trang, deterministic, accuracy 0.72. Không cần cài thêm gì ngoài Java.
- ✅ **hybrid mode** — 0.43s/trang, accuracy **0.90 (#1 benchmark)**. Tốt cho bảng phức tạp,
  PDF scan, công thức. Cần `pip install "opendataloader-pdf[hybrid]"` và chạy server riêng —
  app có nút **▶️ Khởi động server** ngay trong panel Loader.
- ✅ Đọc được **native structure tags** của Tagged PDF (`use_struct_tree`) → reading order chính xác tuyệt đối.
- ✅ Hybrid mode có OCR tích hợp, bật `--force-ocr --ocr-lang "vi,en"` cho PDF scan tiếng Việt.
- ❌ **Bắt buộc có Java 11+** trên máy. Kiểm tra bằng `java -version`, tải tại https://adoptium.net/
- 📐 Phù hợp: corpus lớn cần độ chính xác cao mà máy không có GPU.
""")
...
""")
    ...
    if process_btn:
        ...
        st.session_state["_last_chunk_skip"]     = bool(
            st.session_state.get("chunk_skip", False))
