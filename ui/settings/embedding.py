...
def render_embedding_settings(local_only: bool = False, force_skip: bool = False) -> dict:
    ...
    # ── Bỏ qua embedding ────────────────────────────────────────────────────
    # KHÔNG truyền `value=`: khi đã có `key=`, session_state là nguồn sự thật
    # duy nhất. Truyền cả hai sẽ sinh cảnh báo "created with a default value
    # but also had its value set via the Session State API".
    # force_skip → ghi thẳng vào state TRƯỚC khi tạo widget.
    if force_skip:
        st.session_state["emb_skip"] = True
    st.session_state.setdefault("emb_skip", False)
    ...
    if provider == "huggingface":
        ...
        st.session_state.setdefault('sel_emb_dtype', 'auto')
        ...
        st.session_state.setdefault('sel_emb_batch', _batch_default)
    ...
    # Ollama options
        ...
        st.session_state.setdefault('ollama_emb_url', 'http://localhost:11434')
    ...
    st.session_state.setdefault('emb_enable_sparse', False)
    ...
    if _embed_all:
        ...
        st.session_state.setdefault('emb_max_preview', EMBED_PREVIEW_LIMIT)
