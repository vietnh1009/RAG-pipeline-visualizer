...
def render_vector_db_settings(local_only: bool = False, force_skip: bool = False) -> dict:
    ...
    # Xem chú thích tương ứng ở ui/settings/embedding.py — không truyền `value=`.
    if force_skip:
        st.session_state["vdb_skip"] = True
    st.session_state.setdefault("vdb_skip", False)
    ...
    st.session_state.setdefault('vdb_force_reindex', False)
    ...
    # Qdrant extras
    if provider == "qdrant":
        st.session_state.setdefault('vdb_qdrant_url', _get_env('QDRANT_URL') or 'http://localhost:6333')
        url = st.text_input(
            "Qdrant URL",
            key="vdb_qdrant_url",
            help=":memory: cho in-process, http://localhost:6333 cho Docker, hoặc Qdrant Cloud URL.",
        )
        st.session_state.setdefault('vdb_qdrant_on_disk', False)
        on_disk = st.checkbox(
    ...
    # Weaviate extras
        ...
        st.session_state.setdefault('vdb_weaviate_url', _get_env('WEAVIATE_URL') or 'http://localhost:8080')
        url = st.text_input(
            "Weaviate URL",
    ...
    # pgvector extras
        ...
        st.session_state.setdefault('vdb_pg_conn', _get_env('DATABASE_URL') or '')
    ...
    # Pinecone extras
        ...
        if "cloud" in params:
            ...
            with col2:
                st.session_state.setdefault('vdb_pc_region', 'us-east-1')
                region = st.text_input("Region",key="vdb_pc_region")
            cfg["cloud"]  = cloud
            cfg["region"] = region
