...
def render_chunking_settings(local_only: bool = False) -> tuple[str, int, int, dict]:
    ...
    # ── Bỏ qua chunking ─────────────────────────────────────────────────────
    # Dùng khi chỉ muốn kiểm tra Loader đọc tài liệu ra sao. Không truyền
    # `value=` — chỉ `key=` là đủ, tránh cảnh báo "default value + Session State".
    skip = st.checkbox(
        "Bỏ qua bước Chunking",
        key="chunk_skip",
        help="Chỉ chạy Loader để xem tài liệu được đọc ra thế nào. "
             "Embedding và Vector DB sẽ tự động bị bỏ qua theo.",
    )
    if skip:
        st.caption("⏭️ Pipeline sẽ dừng sau bước Loader.")
        # Trả tuple hợp lệ để caller không phải đổi cách unpack;
        # cờ thật nằm ở extra["skip"] và ở session_state["chunk_skip"].
        return "recursive", 1000, 150, {"skip": True}
    ...
    if strategy == "token_based":
        ...
        if provider == "ollama":
            st.session_state.setdefault('ollama_embed_url', 'http://localhost:11434/v1')
        ...
        st.session_state.setdefault('sel_split_large_sections', _suggested_split)
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.session_state.setdefault('fa_chunk_size', 1000)
            chunk_size = st.number_input(
                "Chunk size (chars)",
                min_value=50, max_value=8000,step=50,
                disabled=not _split_on,
                key="fa_chunk_size",
                help="Kích thước tối đa mỗi sub-chunk sau khi chia nhỏ section"
            )
        with col2:
            st.session_state.setdefault('fa_chunk_overlap', 100)
            chunk_overlap = st.number_input(
                "Chunk overlap (chars)",
