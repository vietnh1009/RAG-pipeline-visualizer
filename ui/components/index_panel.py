...
from core.cache_helpers import (
    ...
    # Kết nối read-only tới store đã persist — định nghĩa DUY NHẤT ở
    # cache_helpers. Trước đây file này giữ một bản sao y hệt (101 dòng).
    _get_chroma_client, _load_vector_store_readonly,
)
    ...
    for entry in valid[_start: _start + _PAGE_SIZE]:
        ...
        with st.container(border=True):
            ...
            with bcol:
                ...
                if st.button("Load →", key=btn_key, type="primary", width="stretch"):
                    from pipeline_cache import PipelineCache
