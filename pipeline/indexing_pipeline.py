...
class IndexingPipeline:
    ...
    def _run_loader(
        ...
        try:
            from core.pipeline_runners import run_loader
            docs = run_loader(
    ...
    def _run_chunking(
        ...
        try:
            from core.pipeline_runners import run_chunker
            chunks = run_chunker(
                docs          = docs,
                strategy      = chunking_cfg["strategy"],
                chunk_size    = chunking_cfg["chunk_size"],
    ...
    def _run_embedding(
        ...
        try:
            from core.pipeline_runners import run_embedder
    ...
    def _run_vector_db(
        ...
        if cached_vdb is not None and not vdb_cfg.get("force_reindex"):
            ...
            try:
                ...
                from core.pipeline_runners import run_vector_db
...
def _cli():
    ...
    # Console Windows mặc định là cp1252 → mọi thông báo tiến độ tiếng Việt
    # bên dưới sẽ ném UnicodeEncodeError và giết CLI ngay ở bước loader.
    # Chỉ ép ở đường CLI, không đụng tới tiến trình Streamlit.
    ...
    # config.yaml LỒNG loader/chunking/embedding/vector_db dưới `indexing:`
    # (giống chunk_documents_from_config). Trước đây _cli() đọc thẳng ở cấp cao
    # nhất → KeyError: 'loader' với đúng file config.yaml mà repo ship.
    # Chấp nhận cả hai bố cục: ưu tiên `indexing:`, lùi về cấp cao nhất.
    idx_cfg = cfg.get("indexing", cfg)

    def _section(name: str) -> dict:
        """Lấy một section indexing, chấp nhận cả hai bố cục config."""
        sec = idx_cfg.get(name, cfg.get(name))
        if sec is None:
            raise KeyError(
                f"Không tìm thấy section '{name}' trong config. "
                f"Đặt nó dưới `indexing:` (khuyến nghị) hoặc ở cấp cao nhất."
            )
        return sec

    loader_sec, chunk_sec = _section("loader"),    _section("chunking")
    embed_sec,  vdb_sec   = _section("embedding"), _section("vector_db")

    # Build cfg dicts từ config.yaml
    loader_cfg = {
        "pdf_strategy":   loader_sec["pdf_strategy"],
        "extract_tables": loader_sec.get("extract_tables", True),
        "language":       cfg.get("data", {}).get("language", "both"),
        "describe_images": loader_sec.get("describe_images", False),
        "vision_model":   loader_sec.get("vision_model", ""),
        "vision_provider": loader_sec.get("vision_provider", "openai"),
        "ollama_base_url": loader_sec.get("ollama_base_url", "http://localhost:11434"),
        "marker_device":  loader_sec.get("marker_device", "cpu"),
    }
    chunking_cfg = {
        "strategy":      chunk_sec["strategy"],
        "chunk_size":    chunk_sec.get("chunk_size", 1000),
        "chunk_overlap": chunk_sec.get("chunk_overlap", 150),
        "extra_kwargs":  {},
    }
    embed_cfg = {
        "provider":       embed_sec["provider"],
        "model_name":     embed_sec["model_name"],
        "enable_sparse":  embed_sec.get("sparse_method", "none") != "none",
        "sparse_method":  embed_sec.get("sparse_method", "none"),
        "skip":           False,
    }
    vdb_cfg = {
        "provider":        vdb_sec["provider"],
        "collection_name": vdb_sec.get("collection_name", "rag_docs"),
        # Tên key phải là `persist_dir` — đúng với chữ ký của các *VectorStore
        # (ChromaVectorStore(persist_dir=...), FAISS...). Dùng "persist_directory"
        # sẽ ném TypeError: unexpected keyword argument ngay ở factory.
        "persist_dir":     vdb_sec.get("persist_dir")
                           or vdb_sec.get("persist_directory", "./storage"),
        "skip":            False,
        "force_reindex":   False,
    }
