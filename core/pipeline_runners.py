...
# run_vector_db() gọi logger.info/logger.warning ở đường ingest theo batch
# (corpus > VDB_BATCH_THRESHOLD = 2000 chunk). Thiếu dòng này, đúng nhánh
# corpus-lớn sẽ ném NameError: name 'logger' is not defined.
...
# ═════════════════════════════════════════════════════════════════════════════
# run_vector_db — bước 4 của indexing stage.
#
# Trước đây hàm này KHÔNG tồn tại: pipeline/indexing_pipeline.py import nó từ
# module `app_visualizer` (module này không có trong repo) nên toàn bộ đường
# chạy headless/CLI đều hỏng. Đây là phần bổ sung.
#
# Khác biệt so với đường UI: hỗ trợ INDEX THEO BATCH. Với corpus lớn,
# FAISS.from_documents() / Chroma.from_documents() gọi embed_documents() trên
# TOÀN BỘ chunk trong một lần — với 100k chunk qua API sẽ vỡ (quá payload
# limit, quá rate limit, và mất sạch nếu lỗi giữa chừng). Batch + retry giải
# quyết cả ba.
# ═════════════════════════════════════════════════════════════════════════════

# Trên ngưỡng này thì chuyển sang chế độ index theo batch.
VDB_BATCH_THRESHOLD = 2_000
VDB_DEFAULT_BATCH   = 500
...
def run_vector_db(
    ...
    sparse_index=None,
    vdb_cfg:         dict | None = None,
    force_reindex:   bool = False,
    on_progress=None,
) -> dict:
    """
    Build (hoặc load lại) vector store và trả về metadata cho pipeline cache.

    Tham số
    -------
    chunks          : list[Document] từ bước chunking.
    embedder_kwargs : dict từ ``_embedder_kwargs_from_cfg(emb_cfg)``.
    sparse_index    : giữ chỗ cho BM25/SPLADE index (chưa dùng).
    vdb_cfg         : dict cấu hình vector DB (provider, collection_name, ...).
                      Key riêng của hàm này:
                        batch_size : int  — số chunk mỗi lần ghi (0 = tắt batch)
    force_reindex   : xoá index cũ, dựng lại từ đầu.
    on_progress     : callable(done:int, total:int) — gọi sau mỗi batch.

    Trả về
    ------
    dict giống hệt đường UI: store / vector_store / n_vectors / collection_name
    / provider / persist_dir / loaded_from_existing.
    """
    from vector_db.factory import get_vector_store
    from embedding.factory import get_embedder

    vdb_cfg = dict(vdb_cfg or {})
    provider = vdb_cfg.get("provider", "chroma")

    # Các key điều khiển của tầng app, không phải kwargs của store class
    _CONTROL_KEYS = {"provider", "skip", "force_reindex", "batch_size"}
    conn_kwargs = {k: v for k, v in vdb_cfg.items() if k not in _CONTROL_KEYS}

    embedder = get_embedder(**embedder_kwargs)

    batch_size = vdb_cfg.get("batch_size")
    if batch_size is None:
        batch_size = VDB_DEFAULT_BATCH if len(chunks) > VDB_BATCH_THRESHOLD else 0

    def _result(store, n):
        return {
            "store":                store,
            "vector_store":         store,      # alias cho đường load index
            "n_vectors":            n,
    ...
    # ── Đường thường: corpus nhỏ, giữ nguyên hành vi như UI ──────────────────
    if not batch_size or len(chunks) <= batch_size:
        store = get_vector_store(
            provider=provider, chunks=chunks, embedder=embedder,
            force_reindex=force_reindex, **conn_kwargs,
        )
        return _result(store, len(chunks))

    # ── Đường corpus lớn: dựng bằng batch đầu, phần còn lại add dần ─────────
    logger.info("Vector DB '%s': index %d chunk theo batch %d.",
                provider, len(chunks), batch_size)

    head, tail = chunks[:batch_size], chunks[batch_size:]
    store = get_vector_store(
        provider=provider, chunks=head, embedder=embedder,
        force_reindex=True,          # batch đầu luôn dựng mới, tránh trộn index cũ
        **conn_kwargs,
    )
    done = len(head)
    if on_progress:
    ...
    _sanitize = BaseVectorStore.sanitize_metadata

    for i in range(0, len(tail), batch_size):
        batch = tail[i: i + batch_size]
        last_exc = None
        for attempt in range(3):                       # retry: lỗi mạng/rate limit
            try:
                store.add_documents(_sanitize(batch))
                last_exc = None
                break
            except Exception as exc:                   # noqa: BLE001
                last_exc = exc
                wait = 2 ** attempt
                logger.warning("Batch %d lỗi (lần %d/3): %s — thử lại sau %ds",
                               i // batch_size + 1, attempt + 1, exc, wait)
                import time as _t
                _t.sleep(wait)
        if last_exc is not None:
            raise RuntimeError(
                f"Index thất bại ở batch bắt đầu tại chunk {done}. "
                f"{done}/{len(chunks)} chunk đã ghi thành công. "
                f"Lỗi gốc: {last_exc}"
            ) from last_exc
        ...
        if on_progress:
            on_progress(done, len(chunks))
        logger.info("Vector DB: %d/%d chunk", done, len(chunks))

    # FAISS chỉ nằm trong RAM — phải save thủ công sau khi add xong
    persist_dir = conn_kwargs.get("persist_dir")
    if provider == "faiss" and persist_dir and hasattr(store, "save_local"):
        from pathlib import Path as _P
        _P(persist_dir).mkdir(parents=True, exist_ok=True)
        store.save_local(str(persist_dir))
        try:
            from vector_db.utils import corpus_fingerprint, save_fingerprint
            save_fingerprint(corpus_fingerprint(chunks),
                             str(_P(persist_dir) / "fingerprint.json"))
        except Exception:      # fingerprint chỉ để tối ưu, không critical
            pass

    return _result(store, done)
